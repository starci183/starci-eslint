...
const MODAL_BRANCH = "D:/repo/src/components/branches/ModalBranch/index.tsx"
...
test("VENDOR-1: the library belongs to closed primitives and named mechanics or surface branches", () => {
  tester.run("vendor-boundary", vendorBoundary, {
    valid: [
      { filename: LEAF, code: "import { Button } from \"@heroui/react\"" },
      { filename: MODAL_BRANCH, code: "import { Modal } from \"@heroui/react\"" },
...
test("VENDOR-2: a mechanics branch wraps its vendor and the legacy shell tier is forbidden", () => {
  tester.run("vendor-boundary", vendorBoundary, {
    valid: [
      { filename: MODAL_BRANCH, code: "import { Modal } from \"@heroui/react\"" },
      {
...
test("VENDOR-6: ModalBranch owns one zero-inset scroll body", () => {
  tester.run("modal-branch-owns-scroll-body", modalBranchOwnsScrollBody, {
    valid: [{
      filename: MODAL_BRANCH,
      code: "export const S = ({ render: Content }) => <Modal.Dialog><Modal.Body className='p-0'><Content /></Modal.Body></Modal.Dialog>",
    }],
    invalid: [
      {
        filename: MODAL_BRANCH,
        code: "export const S = ({ render: Content }) => <Modal.Dialog><Content /></Modal.Dialog>",
        errors: [{ messageId: "missing" }],
      },
      {
        filename: MODAL_BRANCH,
        code: "export const S = ({ render: Content }) => <Modal.Dialog><Modal.Body><Content /></Modal.Body></Modal.Dialog>",
