...
/** Named mechanics branches own the closed vendor interaction machines. */
const MECHANICS_BRANCH = /\/src\/components\/branches\/(?:ModalBranch|DrawerBranch|DropdownBranch)\//
const LEGACY_SHELL_DIR = "/src/components/shells/"
...
export const vendorBoundary = {
  meta: {
    type: "problem",
    docs: { description: "Vendor imports belong to leaves and named mechanics or surface branches." },
    schema: [],
    messages: {
      outside:
      ...
      legacyShell:
        "`components/shells` is a deleted structural tier. Move vendor mechanics into the named Branch owner; framework route content closes outside the component tier into a named contract projection.",
    },
  },
  create(context) {
    ...
    const isLegacyShell = file.includes(LEGACY_SHELL_DIR)
    const isMechanicsBranch = MECHANICS_BRANCH.test(file)
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        if (isLegacyShell) context.report({ node, messageId: "legacyShell" })
        else if (isMechanicsBranch && !importsVendor) context.report({ node, messageId: "emptyBranch" })
      },
    }
  },
}
...
/** The modal mechanics branch owns one zero-inset vendor body as the scroll region. */
export const modalBranchOwnsScrollBody = {
  meta: {
    type: "problem",
    docs: { description: "ModalBranch keeps one zero-inset Modal.Body as its scroll region." },
    schema: [],
    messages: {
      missing:
        "ModalBranch must keep one `Modal.Body` around its named content projection: that vendor body is the dialog's scroll region, not a second content surface.",
      inset:
...
export const accountControlOwnsDropdown = {
  meta: {
    type: "problem",
    docs: { description: "AccountMenu is a block over DropdownBranch; only the branch imports HeroUI Dropdown." },
    schema: [],
    messages: {
      dropdown: "DropdownBranch must import HeroUI `Dropdown`; vendor mechanics belong to the branch.",
      ...
      pieces: "AccountMenu passes typed section data to DropdownBranch; it must not import Branch Section or Item pieces and rebuild vendor anatomy itself.",
    },
  },
  create(context) {
    const file = normalizePath(context.filename || context.getFilename())
 ...
 * half to be silent until named mechanics branches exist. Creating those owners is the migration;
 * no anonymous wrapper tier is introduced.
 */
